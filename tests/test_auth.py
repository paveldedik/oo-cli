...
def oo_home(tmp_path, monkeypatch):
...
def _browser(**params):
    ...
    def open_url(url):
        ...
        answer = "&".join(f"{k}={quote(v)}" for k, v in {"state": query["state"][0], **params}.items())
        _swallow(f"http://127.0.0.1:{query['port'][0]}/callback?{answer}")
        return True
...
def test_the_browser_hands_over_every_cookie_the_gateway_set(monkeypatch):
    monkeypatch.setattr(
        auth.webbrowser,
        "open",
...
def test_the_cli_does_not_care_what_the_gateway_calls_its_cookie(monkeypatch):
    monkeypatch.setattr(auth.webbrowser, "open", _browser(c0="_oauth2_proxy=abc"))
    assert auth.login("https://monitoring.example", LOGIN_PATH, timeout=10) == {"_oauth2_proxy": "abc"}


def test_a_reply_with_the_wrong_state_is_ignored(monkeypatch):
    def open_url(url):
...
def test_an_error_from_the_helper_is_reported(monkeypatch):
    monkeypatch.setattr(auth.webbrowser, "open", _browser(error="no session cookie"))
    with pytest.raises(auth.LoginError, match="no session cookie"):
        auth.login("https://monitoring.example", LOGIN_PATH, timeout=10)


def test_a_browser_that_will_not_open_says_which_url_to_visit(monkeypatch):
    monkeypatch.setattr(auth.webbrowser, "open", lambda url: False)
    with pytest.raises(auth.LoginError, match="cli-login"):
        auth.login("https://monitoring.example", LOGIN_PATH, timeout=10)


def test_a_pasted_cookie_header_is_read_as_it_comes():
    assert auth.parse_cookies("a=1; b=two=2  ") == {"a": "1", "b": "two=2"}


def test_a_paste_without_a_pair_is_an_error():
    with pytest.raises(auth.LoginError):
        auth.parse_cookies("just-a-value")


def test_a_session_is_stored_per_endpoint_and_only_for_the_user(oo_home):
    ...
    assert json.loads(oo_home.joinpath("session.json").read_text()).keys() == {"https://two.example"}


def test_the_cookie_header_is_stable():
