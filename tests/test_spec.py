...
def test_prefers_v2_when_the_endpoint_exists_in_both():
    resolution = Spec(TEMPLATES).resolve("default", ["alerts"], "get")
    assert resolution.path == "/api/v2/default/alerts"
    assert resolution.matched


def test_falls_back_to_v1_when_v2_is_missing():
    resolution = Spec(TEMPLATES).resolve("default", ["dashboards"], "get")
    assert resolution.path == "/api/default/dashboards"
    assert resolution.matched


def test_matches_a_path_parameter():
    resolution = Spec(TEMPLATES).resolve("skippay", ["streams", "sp_metrics"], "get")
    assert resolution.path == "/api/skippay/streams/sp_metrics"
    assert resolution.matched


def test_version_follows_the_method():
    # DELETE only exists on the v2 collection item, POST only on the v1 collection.
    assert Spec(TEMPLATES).resolve("default", ["alerts", "7"], "delete").path == "/api/v2/default/alerts/7"


def test_unknown_resource_is_reported_as_unmatched_v1():
...
def test_without_a_spec_only_the_known_v2_resources_go_to_v2():
    empty = Spec({})
...
def test_an_endpoint_outside_an_organization_drops_the_org():
